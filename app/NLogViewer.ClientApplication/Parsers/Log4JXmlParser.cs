...
namespace NLogViewer.ClientApplication.Parsers
{
    ...
    public class Log4JXmlParser : IDisposable
    {
        ...
        public LogEventInfo? Parse(string xml)
        {
            try
            {
                // If XML doesn't have namespace declaration, add it to make parsing work
                if (!xml.Contains("xmlns:log4j") && !xml.Contains("xmlns="))
                {
                    // Add default namespace declaration if missing
                    var log4jNamespace = "http://jakarta.apache.org/log4j/";
                    if (xml.StartsWith("<log4j:"))
                    {
                        // Find the first > after <log4j:event
                        var eventIndex = xml.IndexOf("<log4j:event");
                        if (eventIndex >= 0)
                        {
                            var firstBracket = xml.IndexOf('>', eventIndex);
                            if (firstBracket > 0)
                            {
                                xml = xml.Insert(firstBracket, $" xmlns:log4j=\"{log4jNamespace}\"");
                            }
                        }
                    }
                }
        ...
        public List<LogEventInfo> ParseMultiple(string xml)
        {
            ...
            try
            {
                ...
                if (!xml.Contains("xmlns:log4j") && !xml.Contains("xmlns="))
                {
                    ...
                    if (xml.StartsWith("<log4j:"))
                    {
                        var eventIndex = xml.IndexOf("<log4j:");
                        if (eventIndex >= 0)
                        {
                            var firstBracket = xml.IndexOf('>', eventIndex);
                            if (firstBracket > 0)
        ...
        private LogEventInfo? ParseEventElement(XElement eventElement)
        {
            try
            {
                ...
                foreach (var prop in properties)
                {
                    ...
                    // Try to get value from attribute first, then from element content
